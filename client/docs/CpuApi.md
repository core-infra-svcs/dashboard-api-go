...
func main() {
    ...
    resp, r, err := apiClient.CpuApi.GetOrganizationWirelessDevicesSystemCpuLoadHistory(context.Background(), organizationId).T0(t0).T1(t1).Timespan(timespan).PerPage(perPage).StartingAfter(startingAfter).EndingBefore(endingBefore).NetworkIds(networkIds).Serials(serials).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `CpuApi.GetOrganizationWirelessDevicesSystemCpuLoadHistory``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `GetOrganizationWirelessDevicesSystemCpuLoadHistory`: InlineResponse200381
    fmt.Fprintf(os.Stdout, "Response from `CpuApi.GetOrganizationWirelessDevicesSystemCpuLoadHistory`: %v\n", resp)
}
```
