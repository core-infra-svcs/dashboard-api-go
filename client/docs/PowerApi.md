...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.PowerApi.GetOrganizationWirelessDevicesPowerModeHistory(context.Background(), organizationId).T0(t0).T1(t1).Timespan(timespan).PerPage(perPage).StartingAfter(startingAfter).EndingBefore(endingBefore).NetworkIds(networkIds).Serials(serials).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `PowerApi.GetOrganizationWirelessDevicesPowerModeHistory``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `GetOrganizationWirelessDevicesPowerModeHistory`: InlineResponse200350
    fmt.Fprintf(os.Stdout, "Response from `PowerApi.GetOrganizationWirelessDevicesPowerModeHistory`: %v\n", resp)
}
```
