/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 03 September, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.62.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200318Network Information about the network that the sensor and gateway are in.
type InlineResponse200318Network struct {
	// The name of the network.
	Name string `json:"name"`
	// The id of the network.
	Id string `json:"id"`
}

// NewInlineResponse200318Network instantiates a new InlineResponse200318Network object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200318Network(name string, id string) *InlineResponse200318Network {
	this := InlineResponse200318Network{}
	this.Name = name
	this.Id = id
	return &this
}

// NewInlineResponse200318NetworkWithDefaults instantiates a new InlineResponse200318Network object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200318NetworkWithDefaults() *InlineResponse200318Network {
	this := InlineResponse200318Network{}
	return &this
}

// GetName returns the Name field value
func (o *InlineResponse200318Network) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *InlineResponse200318Network) GetNameOk() (*string, bool) {
	if o == nil {
    return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *InlineResponse200318Network) SetName(v string) {
	o.Name = v
}

// GetId returns the Id field value
func (o *InlineResponse200318Network) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *InlineResponse200318Network) GetIdOk() (*string, bool) {
	if o == nil {
    return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *InlineResponse200318Network) SetId(v string) {
	o.Id = v
}

func (o InlineResponse200318Network) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["name"] = o.Name
	}
	if true {
		toSerialize["id"] = o.Id
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200318Network struct {
	value *InlineResponse200318Network
	isSet bool
}

func (v NullableInlineResponse200318Network) Get() *InlineResponse200318Network {
	return v.value
}

func (v *NullableInlineResponse200318Network) Set(val *InlineResponse200318Network) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200318Network) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200318Network) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200318Network(val *InlineResponse200318Network) *NullableInlineResponse200318Network {
	return &NullableInlineResponse200318Network{value: val, isSet: true}
}

func (v NullableInlineResponse200318Network) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200318Network) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


