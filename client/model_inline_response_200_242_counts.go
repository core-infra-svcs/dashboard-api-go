/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 07 January, 2026 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.66.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200242Counts counts
type InlineResponse200242Counts struct {
	ByStatus *InlineResponse200242CountsByStatus `json:"byStatus,omitempty"`
}

// NewInlineResponse200242Counts instantiates a new InlineResponse200242Counts object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200242Counts() *InlineResponse200242Counts {
	this := InlineResponse200242Counts{}
	return &this
}

// NewInlineResponse200242CountsWithDefaults instantiates a new InlineResponse200242Counts object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200242CountsWithDefaults() *InlineResponse200242Counts {
	this := InlineResponse200242Counts{}
	return &this
}

// GetByStatus returns the ByStatus field value if set, zero value otherwise.
func (o *InlineResponse200242Counts) GetByStatus() InlineResponse200242CountsByStatus {
	if o == nil || isNil(o.ByStatus) {
		var ret InlineResponse200242CountsByStatus
		return ret
	}
	return *o.ByStatus
}

// GetByStatusOk returns a tuple with the ByStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200242Counts) GetByStatusOk() (*InlineResponse200242CountsByStatus, bool) {
	if o == nil || isNil(o.ByStatus) {
    return nil, false
	}
	return o.ByStatus, true
}

// HasByStatus returns a boolean if a field has been set.
func (o *InlineResponse200242Counts) HasByStatus() bool {
	if o != nil && !isNil(o.ByStatus) {
		return true
	}

	return false
}

// SetByStatus gets a reference to the given InlineResponse200242CountsByStatus and assigns it to the ByStatus field.
func (o *InlineResponse200242Counts) SetByStatus(v InlineResponse200242CountsByStatus) {
	o.ByStatus = &v
}

func (o InlineResponse200242Counts) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.ByStatus) {
		toSerialize["byStatus"] = o.ByStatus
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200242Counts struct {
	value *InlineResponse200242Counts
	isSet bool
}

func (v NullableInlineResponse200242Counts) Get() *InlineResponse200242Counts {
	return v.value
}

func (v *NullableInlineResponse200242Counts) Set(val *InlineResponse200242Counts) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200242Counts) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200242Counts) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200242Counts(val *InlineResponse200242Counts) *NullableInlineResponse200242Counts {
	return &NullableInlineResponse200242Counts{value: val, isSet: true}
}

func (v NullableInlineResponse200242Counts) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200242Counts) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


