/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 06 August, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.61.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200341Counts Number of clients on the port in a given time.
type InlineResponse200341Counts struct {
	ByStatus *InlineResponse200341CountsByStatus `json:"byStatus,omitempty"`
}

// NewInlineResponse200341Counts instantiates a new InlineResponse200341Counts object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200341Counts() *InlineResponse200341Counts {
	this := InlineResponse200341Counts{}
	return &this
}

// NewInlineResponse200341CountsWithDefaults instantiates a new InlineResponse200341Counts object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200341CountsWithDefaults() *InlineResponse200341Counts {
	this := InlineResponse200341Counts{}
	return &this
}

// GetByStatus returns the ByStatus field value if set, zero value otherwise.
func (o *InlineResponse200341Counts) GetByStatus() InlineResponse200341CountsByStatus {
	if o == nil || isNil(o.ByStatus) {
		var ret InlineResponse200341CountsByStatus
		return ret
	}
	return *o.ByStatus
}

// GetByStatusOk returns a tuple with the ByStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200341Counts) GetByStatusOk() (*InlineResponse200341CountsByStatus, bool) {
	if o == nil || isNil(o.ByStatus) {
    return nil, false
	}
	return o.ByStatus, true
}

// HasByStatus returns a boolean if a field has been set.
func (o *InlineResponse200341Counts) HasByStatus() bool {
	if o != nil && !isNil(o.ByStatus) {
		return true
	}

	return false
}

// SetByStatus gets a reference to the given InlineResponse200341CountsByStatus and assigns it to the ByStatus field.
func (o *InlineResponse200341Counts) SetByStatus(v InlineResponse200341CountsByStatus) {
	o.ByStatus = &v
}

func (o InlineResponse200341Counts) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.ByStatus) {
		toSerialize["byStatus"] = o.ByStatus
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200341Counts struct {
	value *InlineResponse200341Counts
	isSet bool
}

func (v NullableInlineResponse200341Counts) Get() *InlineResponse200341Counts {
	return v.value
}

func (v *NullableInlineResponse200341Counts) Set(val *InlineResponse200341Counts) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200341Counts) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200341Counts) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200341Counts(val *InlineResponse200341Counts) *NullableInlineResponse200341Counts {
	return &NullableInlineResponse200341Counts{value: val, isSet: true}
}

func (v NullableInlineResponse200341Counts) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200341Counts) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


