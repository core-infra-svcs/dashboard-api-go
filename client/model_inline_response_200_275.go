/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 07 January, 2026 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.66.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200275 struct for InlineResponse200275
type InlineResponse200275 struct {
	Usage *InlineResponse200275Usage `json:"usage,omitempty"`
	Counts *InlineResponse200275Counts `json:"counts,omitempty"`
}

// NewInlineResponse200275 instantiates a new InlineResponse200275 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200275() *InlineResponse200275 {
	this := InlineResponse200275{}
	return &this
}

// NewInlineResponse200275WithDefaults instantiates a new InlineResponse200275 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200275WithDefaults() *InlineResponse200275 {
	this := InlineResponse200275{}
	return &this
}

// GetUsage returns the Usage field value if set, zero value otherwise.
func (o *InlineResponse200275) GetUsage() InlineResponse200275Usage {
	if o == nil || isNil(o.Usage) {
		var ret InlineResponse200275Usage
		return ret
	}
	return *o.Usage
}

// GetUsageOk returns a tuple with the Usage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200275) GetUsageOk() (*InlineResponse200275Usage, bool) {
	if o == nil || isNil(o.Usage) {
    return nil, false
	}
	return o.Usage, true
}

// HasUsage returns a boolean if a field has been set.
func (o *InlineResponse200275) HasUsage() bool {
	if o != nil && !isNil(o.Usage) {
		return true
	}

	return false
}

// SetUsage gets a reference to the given InlineResponse200275Usage and assigns it to the Usage field.
func (o *InlineResponse200275) SetUsage(v InlineResponse200275Usage) {
	o.Usage = &v
}

// GetCounts returns the Counts field value if set, zero value otherwise.
func (o *InlineResponse200275) GetCounts() InlineResponse200275Counts {
	if o == nil || isNil(o.Counts) {
		var ret InlineResponse200275Counts
		return ret
	}
	return *o.Counts
}

// GetCountsOk returns a tuple with the Counts field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200275) GetCountsOk() (*InlineResponse200275Counts, bool) {
	if o == nil || isNil(o.Counts) {
    return nil, false
	}
	return o.Counts, true
}

// HasCounts returns a boolean if a field has been set.
func (o *InlineResponse200275) HasCounts() bool {
	if o != nil && !isNil(o.Counts) {
		return true
	}

	return false
}

// SetCounts gets a reference to the given InlineResponse200275Counts and assigns it to the Counts field.
func (o *InlineResponse200275) SetCounts(v InlineResponse200275Counts) {
	o.Counts = &v
}

func (o InlineResponse200275) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Usage) {
		toSerialize["usage"] = o.Usage
	}
	if !isNil(o.Counts) {
		toSerialize["counts"] = o.Counts
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200275 struct {
	value *InlineResponse200275
	isSet bool
}

func (v NullableInlineResponse200275) Get() *InlineResponse200275 {
	return v.value
}

func (v *NullableInlineResponse200275) Set(val *InlineResponse200275) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200275) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200275) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200275(val *InlineResponse200275) *NullableInlineResponse200275 {
	return &NullableInlineResponse200275{value: val, isSet: true}
}

func (v NullableInlineResponse200275) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200275) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


