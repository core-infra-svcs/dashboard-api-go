/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 07 February, 2024 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.43.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse20084Switches struct for InlineResponse20084Switches
type InlineResponse20084Switches struct {
	// Switch serial number
	Serial *string `json:"serial,omitempty"`
	// Switch alternative management IP. To remove a prior IP setting, provide an empty string
	AlternateManagementIp *string `json:"alternateManagementIp,omitempty"`
	// Switch subnet mask must be in IP format. Only and must be specified for Polaris switches
	SubnetMask *string `json:"subnetMask,omitempty"`
	// Switch gateway must be in IP format. Only and must be specified for Polaris switches
	Gateway *string `json:"gateway,omitempty"`
}

// NewInlineResponse20084Switches instantiates a new InlineResponse20084Switches object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse20084Switches() *InlineResponse20084Switches {
	this := InlineResponse20084Switches{}
	return &this
}

// NewInlineResponse20084SwitchesWithDefaults instantiates a new InlineResponse20084Switches object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse20084SwitchesWithDefaults() *InlineResponse20084Switches {
	this := InlineResponse20084Switches{}
	return &this
}

// GetSerial returns the Serial field value if set, zero value otherwise.
func (o *InlineResponse20084Switches) GetSerial() string {
	if o == nil || isNil(o.Serial) {
		var ret string
		return ret
	}
	return *o.Serial
}

// GetSerialOk returns a tuple with the Serial field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20084Switches) GetSerialOk() (*string, bool) {
	if o == nil || isNil(o.Serial) {
    return nil, false
	}
	return o.Serial, true
}

// HasSerial returns a boolean if a field has been set.
func (o *InlineResponse20084Switches) HasSerial() bool {
	if o != nil && !isNil(o.Serial) {
		return true
	}

	return false
}

// SetSerial gets a reference to the given string and assigns it to the Serial field.
func (o *InlineResponse20084Switches) SetSerial(v string) {
	o.Serial = &v
}

// GetAlternateManagementIp returns the AlternateManagementIp field value if set, zero value otherwise.
func (o *InlineResponse20084Switches) GetAlternateManagementIp() string {
	if o == nil || isNil(o.AlternateManagementIp) {
		var ret string
		return ret
	}
	return *o.AlternateManagementIp
}

// GetAlternateManagementIpOk returns a tuple with the AlternateManagementIp field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20084Switches) GetAlternateManagementIpOk() (*string, bool) {
	if o == nil || isNil(o.AlternateManagementIp) {
    return nil, false
	}
	return o.AlternateManagementIp, true
}

// HasAlternateManagementIp returns a boolean if a field has been set.
func (o *InlineResponse20084Switches) HasAlternateManagementIp() bool {
	if o != nil && !isNil(o.AlternateManagementIp) {
		return true
	}

	return false
}

// SetAlternateManagementIp gets a reference to the given string and assigns it to the AlternateManagementIp field.
func (o *InlineResponse20084Switches) SetAlternateManagementIp(v string) {
	o.AlternateManagementIp = &v
}

// GetSubnetMask returns the SubnetMask field value if set, zero value otherwise.
func (o *InlineResponse20084Switches) GetSubnetMask() string {
	if o == nil || isNil(o.SubnetMask) {
		var ret string
		return ret
	}
	return *o.SubnetMask
}

// GetSubnetMaskOk returns a tuple with the SubnetMask field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20084Switches) GetSubnetMaskOk() (*string, bool) {
	if o == nil || isNil(o.SubnetMask) {
    return nil, false
	}
	return o.SubnetMask, true
}

// HasSubnetMask returns a boolean if a field has been set.
func (o *InlineResponse20084Switches) HasSubnetMask() bool {
	if o != nil && !isNil(o.SubnetMask) {
		return true
	}

	return false
}

// SetSubnetMask gets a reference to the given string and assigns it to the SubnetMask field.
func (o *InlineResponse20084Switches) SetSubnetMask(v string) {
	o.SubnetMask = &v
}

// GetGateway returns the Gateway field value if set, zero value otherwise.
func (o *InlineResponse20084Switches) GetGateway() string {
	if o == nil || isNil(o.Gateway) {
		var ret string
		return ret
	}
	return *o.Gateway
}

// GetGatewayOk returns a tuple with the Gateway field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20084Switches) GetGatewayOk() (*string, bool) {
	if o == nil || isNil(o.Gateway) {
    return nil, false
	}
	return o.Gateway, true
}

// HasGateway returns a boolean if a field has been set.
func (o *InlineResponse20084Switches) HasGateway() bool {
	if o != nil && !isNil(o.Gateway) {
		return true
	}

	return false
}

// SetGateway gets a reference to the given string and assigns it to the Gateway field.
func (o *InlineResponse20084Switches) SetGateway(v string) {
	o.Gateway = &v
}

func (o InlineResponse20084Switches) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Serial) {
		toSerialize["serial"] = o.Serial
	}
	if !isNil(o.AlternateManagementIp) {
		toSerialize["alternateManagementIp"] = o.AlternateManagementIp
	}
	if !isNil(o.SubnetMask) {
		toSerialize["subnetMask"] = o.SubnetMask
	}
	if !isNil(o.Gateway) {
		toSerialize["gateway"] = o.Gateway
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse20084Switches struct {
	value *InlineResponse20084Switches
	isSet bool
}

func (v NullableInlineResponse20084Switches) Get() *InlineResponse20084Switches {
	return v.value
}

func (v *NullableInlineResponse20084Switches) Set(val *InlineResponse20084Switches) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse20084Switches) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse20084Switches) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse20084Switches(val *InlineResponse20084Switches) *NullableInlineResponse20084Switches {
	return &NullableInlineResponse20084Switches{value: val, isSet: true}
}

func (v NullableInlineResponse20084Switches) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse20084Switches) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


