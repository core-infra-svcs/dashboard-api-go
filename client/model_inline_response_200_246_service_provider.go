/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 02 April, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.57.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200246ServiceProvider Service Provider information
type InlineResponse200246ServiceProvider struct {
	// Service Provider name
	Name *string `json:"name,omitempty"`
	// Plans currently active on the eSIM
	Plans []InlineResponse200246ServiceProviderPlans `json:"plans,omitempty"`
}

// NewInlineResponse200246ServiceProvider instantiates a new InlineResponse200246ServiceProvider object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200246ServiceProvider() *InlineResponse200246ServiceProvider {
	this := InlineResponse200246ServiceProvider{}
	return &this
}

// NewInlineResponse200246ServiceProviderWithDefaults instantiates a new InlineResponse200246ServiceProvider object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200246ServiceProviderWithDefaults() *InlineResponse200246ServiceProvider {
	this := InlineResponse200246ServiceProvider{}
	return &this
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *InlineResponse200246ServiceProvider) GetName() string {
	if o == nil || isNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200246ServiceProvider) GetNameOk() (*string, bool) {
	if o == nil || isNil(o.Name) {
    return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *InlineResponse200246ServiceProvider) HasName() bool {
	if o != nil && !isNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *InlineResponse200246ServiceProvider) SetName(v string) {
	o.Name = &v
}

// GetPlans returns the Plans field value if set, zero value otherwise.
func (o *InlineResponse200246ServiceProvider) GetPlans() []InlineResponse200246ServiceProviderPlans {
	if o == nil || isNil(o.Plans) {
		var ret []InlineResponse200246ServiceProviderPlans
		return ret
	}
	return o.Plans
}

// GetPlansOk returns a tuple with the Plans field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200246ServiceProvider) GetPlansOk() ([]InlineResponse200246ServiceProviderPlans, bool) {
	if o == nil || isNil(o.Plans) {
    return nil, false
	}
	return o.Plans, true
}

// HasPlans returns a boolean if a field has been set.
func (o *InlineResponse200246ServiceProvider) HasPlans() bool {
	if o != nil && !isNil(o.Plans) {
		return true
	}

	return false
}

// SetPlans gets a reference to the given []InlineResponse200246ServiceProviderPlans and assigns it to the Plans field.
func (o *InlineResponse200246ServiceProvider) SetPlans(v []InlineResponse200246ServiceProviderPlans) {
	o.Plans = v
}

func (o InlineResponse200246ServiceProvider) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !isNil(o.Plans) {
		toSerialize["plans"] = o.Plans
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200246ServiceProvider struct {
	value *InlineResponse200246ServiceProvider
	isSet bool
}

func (v NullableInlineResponse200246ServiceProvider) Get() *InlineResponse200246ServiceProvider {
	return v.value
}

func (v *NullableInlineResponse200246ServiceProvider) Set(val *InlineResponse200246ServiceProvider) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200246ServiceProvider) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200246ServiceProvider) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200246ServiceProvider(val *InlineResponse200246ServiceProvider) *NullableInlineResponse200246ServiceProvider {
	return &NullableInlineResponse200246ServiceProvider{value: val, isSet: true}
}

func (v NullableInlineResponse200246ServiceProvider) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200246ServiceProvider) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


