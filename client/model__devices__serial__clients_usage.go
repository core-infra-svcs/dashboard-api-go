/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 03 December, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.65.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// DevicesSerialClientsUsage Client usage data for sent and received
type DevicesSerialClientsUsage struct {
	// Usage sent by the client
	Sent *float32 `json:"sent,omitempty"`
	// Usage received by the client
	Recv *float32 `json:"recv,omitempty"`
}

// NewDevicesSerialClientsUsage instantiates a new DevicesSerialClientsUsage object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDevicesSerialClientsUsage() *DevicesSerialClientsUsage {
	this := DevicesSerialClientsUsage{}
	return &this
}

// NewDevicesSerialClientsUsageWithDefaults instantiates a new DevicesSerialClientsUsage object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDevicesSerialClientsUsageWithDefaults() *DevicesSerialClientsUsage {
	this := DevicesSerialClientsUsage{}
	return &this
}

// GetSent returns the Sent field value if set, zero value otherwise.
func (o *DevicesSerialClientsUsage) GetSent() float32 {
	if o == nil || isNil(o.Sent) {
		var ret float32
		return ret
	}
	return *o.Sent
}

// GetSentOk returns a tuple with the Sent field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DevicesSerialClientsUsage) GetSentOk() (*float32, bool) {
	if o == nil || isNil(o.Sent) {
    return nil, false
	}
	return o.Sent, true
}

// HasSent returns a boolean if a field has been set.
func (o *DevicesSerialClientsUsage) HasSent() bool {
	if o != nil && !isNil(o.Sent) {
		return true
	}

	return false
}

// SetSent gets a reference to the given float32 and assigns it to the Sent field.
func (o *DevicesSerialClientsUsage) SetSent(v float32) {
	o.Sent = &v
}

// GetRecv returns the Recv field value if set, zero value otherwise.
func (o *DevicesSerialClientsUsage) GetRecv() float32 {
	if o == nil || isNil(o.Recv) {
		var ret float32
		return ret
	}
	return *o.Recv
}

// GetRecvOk returns a tuple with the Recv field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DevicesSerialClientsUsage) GetRecvOk() (*float32, bool) {
	if o == nil || isNil(o.Recv) {
    return nil, false
	}
	return o.Recv, true
}

// HasRecv returns a boolean if a field has been set.
func (o *DevicesSerialClientsUsage) HasRecv() bool {
	if o != nil && !isNil(o.Recv) {
		return true
	}

	return false
}

// SetRecv gets a reference to the given float32 and assigns it to the Recv field.
func (o *DevicesSerialClientsUsage) SetRecv(v float32) {
	o.Recv = &v
}

func (o DevicesSerialClientsUsage) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Sent) {
		toSerialize["sent"] = o.Sent
	}
	if !isNil(o.Recv) {
		toSerialize["recv"] = o.Recv
	}
	return json.Marshal(toSerialize)
}

type NullableDevicesSerialClientsUsage struct {
	value *DevicesSerialClientsUsage
	isSet bool
}

func (v NullableDevicesSerialClientsUsage) Get() *DevicesSerialClientsUsage {
	return v.value
}

func (v *NullableDevicesSerialClientsUsage) Set(val *DevicesSerialClientsUsage) {
	v.value = val
	v.isSet = true
}

func (v NullableDevicesSerialClientsUsage) IsSet() bool {
	return v.isSet
}

func (v *NullableDevicesSerialClientsUsage) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDevicesSerialClientsUsage(val *DevicesSerialClientsUsage) *NullableDevicesSerialClientsUsage {
	return &NullableDevicesSerialClientsUsage{value: val, isSet: true}
}

func (v NullableDevicesSerialClientsUsage) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDevicesSerialClientsUsage) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


