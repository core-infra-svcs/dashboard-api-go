/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 06 August, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.61.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse20111Addresses1 struct for InlineResponse20111Addresses1
type InlineResponse20111Addresses1 struct {
	// Protocol of the interface
	Protocol *string `json:"protocol,omitempty"`
	// Gateway of the interface
	Gateway *string `json:"gateway,omitempty"`
	// Subnet mask of the interface
	SubnetMask *string `json:"subnetMask,omitempty"`
}

// NewInlineResponse20111Addresses1 instantiates a new InlineResponse20111Addresses1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse20111Addresses1() *InlineResponse20111Addresses1 {
	this := InlineResponse20111Addresses1{}
	return &this
}

// NewInlineResponse20111Addresses1WithDefaults instantiates a new InlineResponse20111Addresses1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse20111Addresses1WithDefaults() *InlineResponse20111Addresses1 {
	this := InlineResponse20111Addresses1{}
	return &this
}

// GetProtocol returns the Protocol field value if set, zero value otherwise.
func (o *InlineResponse20111Addresses1) GetProtocol() string {
	if o == nil || isNil(o.Protocol) {
		var ret string
		return ret
	}
	return *o.Protocol
}

// GetProtocolOk returns a tuple with the Protocol field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20111Addresses1) GetProtocolOk() (*string, bool) {
	if o == nil || isNil(o.Protocol) {
    return nil, false
	}
	return o.Protocol, true
}

// HasProtocol returns a boolean if a field has been set.
func (o *InlineResponse20111Addresses1) HasProtocol() bool {
	if o != nil && !isNil(o.Protocol) {
		return true
	}

	return false
}

// SetProtocol gets a reference to the given string and assigns it to the Protocol field.
func (o *InlineResponse20111Addresses1) SetProtocol(v string) {
	o.Protocol = &v
}

// GetGateway returns the Gateway field value if set, zero value otherwise.
func (o *InlineResponse20111Addresses1) GetGateway() string {
	if o == nil || isNil(o.Gateway) {
		var ret string
		return ret
	}
	return *o.Gateway
}

// GetGatewayOk returns a tuple with the Gateway field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20111Addresses1) GetGatewayOk() (*string, bool) {
	if o == nil || isNil(o.Gateway) {
    return nil, false
	}
	return o.Gateway, true
}

// HasGateway returns a boolean if a field has been set.
func (o *InlineResponse20111Addresses1) HasGateway() bool {
	if o != nil && !isNil(o.Gateway) {
		return true
	}

	return false
}

// SetGateway gets a reference to the given string and assigns it to the Gateway field.
func (o *InlineResponse20111Addresses1) SetGateway(v string) {
	o.Gateway = &v
}

// GetSubnetMask returns the SubnetMask field value if set, zero value otherwise.
func (o *InlineResponse20111Addresses1) GetSubnetMask() string {
	if o == nil || isNil(o.SubnetMask) {
		var ret string
		return ret
	}
	return *o.SubnetMask
}

// GetSubnetMaskOk returns a tuple with the SubnetMask field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20111Addresses1) GetSubnetMaskOk() (*string, bool) {
	if o == nil || isNil(o.SubnetMask) {
    return nil, false
	}
	return o.SubnetMask, true
}

// HasSubnetMask returns a boolean if a field has been set.
func (o *InlineResponse20111Addresses1) HasSubnetMask() bool {
	if o != nil && !isNil(o.SubnetMask) {
		return true
	}

	return false
}

// SetSubnetMask gets a reference to the given string and assigns it to the SubnetMask field.
func (o *InlineResponse20111Addresses1) SetSubnetMask(v string) {
	o.SubnetMask = &v
}

func (o InlineResponse20111Addresses1) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Protocol) {
		toSerialize["protocol"] = o.Protocol
	}
	if !isNil(o.Gateway) {
		toSerialize["gateway"] = o.Gateway
	}
	if !isNil(o.SubnetMask) {
		toSerialize["subnetMask"] = o.SubnetMask
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse20111Addresses1 struct {
	value *InlineResponse20111Addresses1
	isSet bool
}

func (v NullableInlineResponse20111Addresses1) Get() *InlineResponse20111Addresses1 {
	return v.value
}

func (v *NullableInlineResponse20111Addresses1) Set(val *InlineResponse20111Addresses1) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse20111Addresses1) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse20111Addresses1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse20111Addresses1(val *InlineResponse20111Addresses1) *NullableInlineResponse20111Addresses1 {
	return &NullableInlineResponse20111Addresses1{value: val, isSet: true}
}

func (v NullableInlineResponse20111Addresses1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse20111Addresses1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


