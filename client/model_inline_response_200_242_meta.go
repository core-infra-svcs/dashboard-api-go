/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 02 July, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.60.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200242Meta Metadata about the response
type InlineResponse200242Meta struct {
	Counts InlineResponse200242MetaCounts `json:"counts"`
}

// NewInlineResponse200242Meta instantiates a new InlineResponse200242Meta object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200242Meta(counts InlineResponse200242MetaCounts) *InlineResponse200242Meta {
	this := InlineResponse200242Meta{}
	this.Counts = counts
	return &this
}

// NewInlineResponse200242MetaWithDefaults instantiates a new InlineResponse200242Meta object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200242MetaWithDefaults() *InlineResponse200242Meta {
	this := InlineResponse200242Meta{}
	return &this
}

// GetCounts returns the Counts field value
func (o *InlineResponse200242Meta) GetCounts() InlineResponse200242MetaCounts {
	if o == nil {
		var ret InlineResponse200242MetaCounts
		return ret
	}

	return o.Counts
}

// GetCountsOk returns a tuple with the Counts field value
// and a boolean to check if the value has been set.
func (o *InlineResponse200242Meta) GetCountsOk() (*InlineResponse200242MetaCounts, bool) {
	if o == nil {
    return nil, false
	}
	return &o.Counts, true
}

// SetCounts sets field value
func (o *InlineResponse200242Meta) SetCounts(v InlineResponse200242MetaCounts) {
	o.Counts = v
}

func (o InlineResponse200242Meta) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["counts"] = o.Counts
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200242Meta struct {
	value *InlineResponse200242Meta
	isSet bool
}

func (v NullableInlineResponse200242Meta) Get() *InlineResponse200242Meta {
	return v.value
}

func (v *NullableInlineResponse200242Meta) Set(val *InlineResponse200242Meta) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200242Meta) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200242Meta) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200242Meta(val *InlineResponse200242Meta) *NullableInlineResponse200242Meta {
	return &NullableInlineResponse200242Meta{value: val, isSet: true}
}

func (v NullableInlineResponse200242Meta) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200242Meta) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


