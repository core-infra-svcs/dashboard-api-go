/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 07 May, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.58.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200276Used Information regarding memory usage on the device over the entire timespan
type InlineResponse200276Used struct {
	// Median memory in kB used on the device over the entire timespan rounded up to nearest integer
	Median *int32 `json:"median,omitempty"`
}

// NewInlineResponse200276Used instantiates a new InlineResponse200276Used object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200276Used() *InlineResponse200276Used {
	this := InlineResponse200276Used{}
	return &this
}

// NewInlineResponse200276UsedWithDefaults instantiates a new InlineResponse200276Used object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200276UsedWithDefaults() *InlineResponse200276Used {
	this := InlineResponse200276Used{}
	return &this
}

// GetMedian returns the Median field value if set, zero value otherwise.
func (o *InlineResponse200276Used) GetMedian() int32 {
	if o == nil || isNil(o.Median) {
		var ret int32
		return ret
	}
	return *o.Median
}

// GetMedianOk returns a tuple with the Median field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200276Used) GetMedianOk() (*int32, bool) {
	if o == nil || isNil(o.Median) {
    return nil, false
	}
	return o.Median, true
}

// HasMedian returns a boolean if a field has been set.
func (o *InlineResponse200276Used) HasMedian() bool {
	if o != nil && !isNil(o.Median) {
		return true
	}

	return false
}

// SetMedian gets a reference to the given int32 and assigns it to the Median field.
func (o *InlineResponse200276Used) SetMedian(v int32) {
	o.Median = &v
}

func (o InlineResponse200276Used) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Median) {
		toSerialize["median"] = o.Median
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200276Used struct {
	value *InlineResponse200276Used
	isSet bool
}

func (v NullableInlineResponse200276Used) Get() *InlineResponse200276Used {
	return v.value
}

func (v *NullableInlineResponse200276Used) Set(val *InlineResponse200276Used) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200276Used) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200276Used) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200276Used(val *InlineResponse200276Used) *NullableInlineResponse200276Used {
	return &NullableInlineResponse200276Used{value: val, isSet: true}
}

func (v NullableInlineResponse200276Used) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200276Used) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


