/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 07 December, 2022 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.28.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse20021Stages struct for InlineResponse20021Stages
type InlineResponse20021Stages struct {
	Group *InlineResponse20021Group `json:"group,omitempty"`
	Milestones *InlineResponse20021Milestones `json:"milestones,omitempty"`
	// Current upgrade status of the group
	Status *string `json:"status,omitempty"`
}

// NewInlineResponse20021Stages instantiates a new InlineResponse20021Stages object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse20021Stages() *InlineResponse20021Stages {
	this := InlineResponse20021Stages{}
	return &this
}

// NewInlineResponse20021StagesWithDefaults instantiates a new InlineResponse20021Stages object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse20021StagesWithDefaults() *InlineResponse20021Stages {
	this := InlineResponse20021Stages{}
	return &this
}

// GetGroup returns the Group field value if set, zero value otherwise.
func (o *InlineResponse20021Stages) GetGroup() InlineResponse20021Group {
	if o == nil || isNil(o.Group) {
		var ret InlineResponse20021Group
		return ret
	}
	return *o.Group
}

// GetGroupOk returns a tuple with the Group field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20021Stages) GetGroupOk() (*InlineResponse20021Group, bool) {
	if o == nil || isNil(o.Group) {
    return nil, false
	}
	return o.Group, true
}

// HasGroup returns a boolean if a field has been set.
func (o *InlineResponse20021Stages) HasGroup() bool {
	if o != nil && !isNil(o.Group) {
		return true
	}

	return false
}

// SetGroup gets a reference to the given InlineResponse20021Group and assigns it to the Group field.
func (o *InlineResponse20021Stages) SetGroup(v InlineResponse20021Group) {
	o.Group = &v
}

// GetMilestones returns the Milestones field value if set, zero value otherwise.
func (o *InlineResponse20021Stages) GetMilestones() InlineResponse20021Milestones {
	if o == nil || isNil(o.Milestones) {
		var ret InlineResponse20021Milestones
		return ret
	}
	return *o.Milestones
}

// GetMilestonesOk returns a tuple with the Milestones field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20021Stages) GetMilestonesOk() (*InlineResponse20021Milestones, bool) {
	if o == nil || isNil(o.Milestones) {
    return nil, false
	}
	return o.Milestones, true
}

// HasMilestones returns a boolean if a field has been set.
func (o *InlineResponse20021Stages) HasMilestones() bool {
	if o != nil && !isNil(o.Milestones) {
		return true
	}

	return false
}

// SetMilestones gets a reference to the given InlineResponse20021Milestones and assigns it to the Milestones field.
func (o *InlineResponse20021Stages) SetMilestones(v InlineResponse20021Milestones) {
	o.Milestones = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *InlineResponse20021Stages) GetStatus() string {
	if o == nil || isNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20021Stages) GetStatusOk() (*string, bool) {
	if o == nil || isNil(o.Status) {
    return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *InlineResponse20021Stages) HasStatus() bool {
	if o != nil && !isNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *InlineResponse20021Stages) SetStatus(v string) {
	o.Status = &v
}

func (o InlineResponse20021Stages) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Group) {
		toSerialize["group"] = o.Group
	}
	if !isNil(o.Milestones) {
		toSerialize["milestones"] = o.Milestones
	}
	if !isNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse20021Stages struct {
	value *InlineResponse20021Stages
	isSet bool
}

func (v NullableInlineResponse20021Stages) Get() *InlineResponse20021Stages {
	return v.value
}

func (v *NullableInlineResponse20021Stages) Set(val *InlineResponse20021Stages) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse20021Stages) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse20021Stages) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse20021Stages(val *InlineResponse20021Stages) *NullableInlineResponse20021Stages {
	return &NullableInlineResponse20021Stages{value: val, isSet: true}
}

func (v NullableInlineResponse20021Stages) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse20021Stages) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


