/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 02 July, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.60.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200303LicenseTypes struct for InlineResponse200303LicenseTypes
type InlineResponse200303LicenseTypes struct {
	// License type
	LicenseType *string `json:"licenseType,omitempty"`
	Counts *InlineResponse200303Counts `json:"counts,omitempty"`
}

// NewInlineResponse200303LicenseTypes instantiates a new InlineResponse200303LicenseTypes object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200303LicenseTypes() *InlineResponse200303LicenseTypes {
	this := InlineResponse200303LicenseTypes{}
	return &this
}

// NewInlineResponse200303LicenseTypesWithDefaults instantiates a new InlineResponse200303LicenseTypes object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200303LicenseTypesWithDefaults() *InlineResponse200303LicenseTypes {
	this := InlineResponse200303LicenseTypes{}
	return &this
}

// GetLicenseType returns the LicenseType field value if set, zero value otherwise.
func (o *InlineResponse200303LicenseTypes) GetLicenseType() string {
	if o == nil || isNil(o.LicenseType) {
		var ret string
		return ret
	}
	return *o.LicenseType
}

// GetLicenseTypeOk returns a tuple with the LicenseType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200303LicenseTypes) GetLicenseTypeOk() (*string, bool) {
	if o == nil || isNil(o.LicenseType) {
    return nil, false
	}
	return o.LicenseType, true
}

// HasLicenseType returns a boolean if a field has been set.
func (o *InlineResponse200303LicenseTypes) HasLicenseType() bool {
	if o != nil && !isNil(o.LicenseType) {
		return true
	}

	return false
}

// SetLicenseType gets a reference to the given string and assigns it to the LicenseType field.
func (o *InlineResponse200303LicenseTypes) SetLicenseType(v string) {
	o.LicenseType = &v
}

// GetCounts returns the Counts field value if set, zero value otherwise.
func (o *InlineResponse200303LicenseTypes) GetCounts() InlineResponse200303Counts {
	if o == nil || isNil(o.Counts) {
		var ret InlineResponse200303Counts
		return ret
	}
	return *o.Counts
}

// GetCountsOk returns a tuple with the Counts field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200303LicenseTypes) GetCountsOk() (*InlineResponse200303Counts, bool) {
	if o == nil || isNil(o.Counts) {
    return nil, false
	}
	return o.Counts, true
}

// HasCounts returns a boolean if a field has been set.
func (o *InlineResponse200303LicenseTypes) HasCounts() bool {
	if o != nil && !isNil(o.Counts) {
		return true
	}

	return false
}

// SetCounts gets a reference to the given InlineResponse200303Counts and assigns it to the Counts field.
func (o *InlineResponse200303LicenseTypes) SetCounts(v InlineResponse200303Counts) {
	o.Counts = &v
}

func (o InlineResponse200303LicenseTypes) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.LicenseType) {
		toSerialize["licenseType"] = o.LicenseType
	}
	if !isNil(o.Counts) {
		toSerialize["counts"] = o.Counts
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200303LicenseTypes struct {
	value *InlineResponse200303LicenseTypes
	isSet bool
}

func (v NullableInlineResponse200303LicenseTypes) Get() *InlineResponse200303LicenseTypes {
	return v.value
}

func (v *NullableInlineResponse200303LicenseTypes) Set(val *InlineResponse200303LicenseTypes) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200303LicenseTypes) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200303LicenseTypes) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200303LicenseTypes(val *InlineResponse200303LicenseTypes) *NullableInlineResponse200303LicenseTypes {
	return &NullableInlineResponse200303LicenseTypes{value: val, isSet: true}
}

func (v NullableInlineResponse200303LicenseTypes) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200303LicenseTypes) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


