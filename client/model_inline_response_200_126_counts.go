/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 05 July, 2023 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.35.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200126Counts counts
type InlineResponse200126Counts struct {
	ByStatus *InlineResponse200126CountsByStatus `json:"byStatus,omitempty"`
}

// NewInlineResponse200126Counts instantiates a new InlineResponse200126Counts object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200126Counts() *InlineResponse200126Counts {
	this := InlineResponse200126Counts{}
	return &this
}

// NewInlineResponse200126CountsWithDefaults instantiates a new InlineResponse200126Counts object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200126CountsWithDefaults() *InlineResponse200126Counts {
	this := InlineResponse200126Counts{}
	return &this
}

// GetByStatus returns the ByStatus field value if set, zero value otherwise.
func (o *InlineResponse200126Counts) GetByStatus() InlineResponse200126CountsByStatus {
	if o == nil || isNil(o.ByStatus) {
		var ret InlineResponse200126CountsByStatus
		return ret
	}
	return *o.ByStatus
}

// GetByStatusOk returns a tuple with the ByStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200126Counts) GetByStatusOk() (*InlineResponse200126CountsByStatus, bool) {
	if o == nil || isNil(o.ByStatus) {
    return nil, false
	}
	return o.ByStatus, true
}

// HasByStatus returns a boolean if a field has been set.
func (o *InlineResponse200126Counts) HasByStatus() bool {
	if o != nil && !isNil(o.ByStatus) {
		return true
	}

	return false
}

// SetByStatus gets a reference to the given InlineResponse200126CountsByStatus and assigns it to the ByStatus field.
func (o *InlineResponse200126Counts) SetByStatus(v InlineResponse200126CountsByStatus) {
	o.ByStatus = &v
}

func (o InlineResponse200126Counts) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.ByStatus) {
		toSerialize["byStatus"] = o.ByStatus
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200126Counts struct {
	value *InlineResponse200126Counts
	isSet bool
}

func (v NullableInlineResponse200126Counts) Get() *InlineResponse200126Counts {
	return v.value
}

func (v *NullableInlineResponse200126Counts) Set(val *InlineResponse200126Counts) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200126Counts) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200126Counts) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200126Counts(val *InlineResponse200126Counts) *NullableInlineResponse200126Counts {
	return &NullableInlineResponse200126Counts{value: val, isSet: true}
}

func (v NullableInlineResponse200126Counts) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200126Counts) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


