/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 05 February, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.55.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200169Servers struct for InlineResponse200169Servers
type InlineResponse200169Servers struct {
	// The IP address of the syslog server
	Host *string `json:"host,omitempty"`
	// The port of the syslog server
	Port *int32 `json:"port,omitempty"`
	// A list of roles for the syslog server. Options (case-insensitive): 'Wireless event log', 'Appliance event log', 'Switch event log', 'Air Marshal events', 'Flows', 'URLs', 'IDS alerts', 'Security events'
	Roles []string `json:"roles,omitempty"`
	Encryption *InlineResponse200169Encryption `json:"encryption,omitempty"`
}

// NewInlineResponse200169Servers instantiates a new InlineResponse200169Servers object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200169Servers() *InlineResponse200169Servers {
	this := InlineResponse200169Servers{}
	return &this
}

// NewInlineResponse200169ServersWithDefaults instantiates a new InlineResponse200169Servers object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200169ServersWithDefaults() *InlineResponse200169Servers {
	this := InlineResponse200169Servers{}
	return &this
}

// GetHost returns the Host field value if set, zero value otherwise.
func (o *InlineResponse200169Servers) GetHost() string {
	if o == nil || isNil(o.Host) {
		var ret string
		return ret
	}
	return *o.Host
}

// GetHostOk returns a tuple with the Host field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200169Servers) GetHostOk() (*string, bool) {
	if o == nil || isNil(o.Host) {
    return nil, false
	}
	return o.Host, true
}

// HasHost returns a boolean if a field has been set.
func (o *InlineResponse200169Servers) HasHost() bool {
	if o != nil && !isNil(o.Host) {
		return true
	}

	return false
}

// SetHost gets a reference to the given string and assigns it to the Host field.
func (o *InlineResponse200169Servers) SetHost(v string) {
	o.Host = &v
}

// GetPort returns the Port field value if set, zero value otherwise.
func (o *InlineResponse200169Servers) GetPort() int32 {
	if o == nil || isNil(o.Port) {
		var ret int32
		return ret
	}
	return *o.Port
}

// GetPortOk returns a tuple with the Port field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200169Servers) GetPortOk() (*int32, bool) {
	if o == nil || isNil(o.Port) {
    return nil, false
	}
	return o.Port, true
}

// HasPort returns a boolean if a field has been set.
func (o *InlineResponse200169Servers) HasPort() bool {
	if o != nil && !isNil(o.Port) {
		return true
	}

	return false
}

// SetPort gets a reference to the given int32 and assigns it to the Port field.
func (o *InlineResponse200169Servers) SetPort(v int32) {
	o.Port = &v
}

// GetRoles returns the Roles field value if set, zero value otherwise.
func (o *InlineResponse200169Servers) GetRoles() []string {
	if o == nil || isNil(o.Roles) {
		var ret []string
		return ret
	}
	return o.Roles
}

// GetRolesOk returns a tuple with the Roles field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200169Servers) GetRolesOk() ([]string, bool) {
	if o == nil || isNil(o.Roles) {
    return nil, false
	}
	return o.Roles, true
}

// HasRoles returns a boolean if a field has been set.
func (o *InlineResponse200169Servers) HasRoles() bool {
	if o != nil && !isNil(o.Roles) {
		return true
	}

	return false
}

// SetRoles gets a reference to the given []string and assigns it to the Roles field.
func (o *InlineResponse200169Servers) SetRoles(v []string) {
	o.Roles = v
}

// GetEncryption returns the Encryption field value if set, zero value otherwise.
func (o *InlineResponse200169Servers) GetEncryption() InlineResponse200169Encryption {
	if o == nil || isNil(o.Encryption) {
		var ret InlineResponse200169Encryption
		return ret
	}
	return *o.Encryption
}

// GetEncryptionOk returns a tuple with the Encryption field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200169Servers) GetEncryptionOk() (*InlineResponse200169Encryption, bool) {
	if o == nil || isNil(o.Encryption) {
    return nil, false
	}
	return o.Encryption, true
}

// HasEncryption returns a boolean if a field has been set.
func (o *InlineResponse200169Servers) HasEncryption() bool {
	if o != nil && !isNil(o.Encryption) {
		return true
	}

	return false
}

// SetEncryption gets a reference to the given InlineResponse200169Encryption and assigns it to the Encryption field.
func (o *InlineResponse200169Servers) SetEncryption(v InlineResponse200169Encryption) {
	o.Encryption = &v
}

func (o InlineResponse200169Servers) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Host) {
		toSerialize["host"] = o.Host
	}
	if !isNil(o.Port) {
		toSerialize["port"] = o.Port
	}
	if !isNil(o.Roles) {
		toSerialize["roles"] = o.Roles
	}
	if !isNil(o.Encryption) {
		toSerialize["encryption"] = o.Encryption
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200169Servers struct {
	value *InlineResponse200169Servers
	isSet bool
}

func (v NullableInlineResponse200169Servers) Get() *InlineResponse200169Servers {
	return v.value
}

func (v *NullableInlineResponse200169Servers) Set(val *InlineResponse200169Servers) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200169Servers) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200169Servers) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200169Servers(val *InlineResponse200169Servers) *NullableInlineResponse200169Servers {
	return &NullableInlineResponse200169Servers{value: val, isSet: true}
}

func (v NullableInlineResponse200169Servers) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200169Servers) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


