/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 07 May, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.58.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200194Api Whether to enable collection of location and scanning analytics.
type InlineResponse200194Api struct {
	// Whether to enable push API for scanning events. Analytics must be enabled to enable scanning API.
	Enabled *bool `json:"enabled,omitempty"`
	Validator *InlineResponse200194ApiValidator `json:"validator,omitempty"`
}

// NewInlineResponse200194Api instantiates a new InlineResponse200194Api object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200194Api() *InlineResponse200194Api {
	this := InlineResponse200194Api{}
	return &this
}

// NewInlineResponse200194ApiWithDefaults instantiates a new InlineResponse200194Api object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200194ApiWithDefaults() *InlineResponse200194Api {
	this := InlineResponse200194Api{}
	return &this
}

// GetEnabled returns the Enabled field value if set, zero value otherwise.
func (o *InlineResponse200194Api) GetEnabled() bool {
	if o == nil || isNil(o.Enabled) {
		var ret bool
		return ret
	}
	return *o.Enabled
}

// GetEnabledOk returns a tuple with the Enabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200194Api) GetEnabledOk() (*bool, bool) {
	if o == nil || isNil(o.Enabled) {
    return nil, false
	}
	return o.Enabled, true
}

// HasEnabled returns a boolean if a field has been set.
func (o *InlineResponse200194Api) HasEnabled() bool {
	if o != nil && !isNil(o.Enabled) {
		return true
	}

	return false
}

// SetEnabled gets a reference to the given bool and assigns it to the Enabled field.
func (o *InlineResponse200194Api) SetEnabled(v bool) {
	o.Enabled = &v
}

// GetValidator returns the Validator field value if set, zero value otherwise.
func (o *InlineResponse200194Api) GetValidator() InlineResponse200194ApiValidator {
	if o == nil || isNil(o.Validator) {
		var ret InlineResponse200194ApiValidator
		return ret
	}
	return *o.Validator
}

// GetValidatorOk returns a tuple with the Validator field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200194Api) GetValidatorOk() (*InlineResponse200194ApiValidator, bool) {
	if o == nil || isNil(o.Validator) {
    return nil, false
	}
	return o.Validator, true
}

// HasValidator returns a boolean if a field has been set.
func (o *InlineResponse200194Api) HasValidator() bool {
	if o != nil && !isNil(o.Validator) {
		return true
	}

	return false
}

// SetValidator gets a reference to the given InlineResponse200194ApiValidator and assigns it to the Validator field.
func (o *InlineResponse200194Api) SetValidator(v InlineResponse200194ApiValidator) {
	o.Validator = &v
}

func (o InlineResponse200194Api) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Enabled) {
		toSerialize["enabled"] = o.Enabled
	}
	if !isNil(o.Validator) {
		toSerialize["validator"] = o.Validator
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200194Api struct {
	value *InlineResponse200194Api
	isSet bool
}

func (v NullableInlineResponse200194Api) Get() *InlineResponse200194Api {
	return v.value
}

func (v *NullableInlineResponse200194Api) Set(val *InlineResponse200194Api) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200194Api) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200194Api) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200194Api(val *InlineResponse200194Api) *NullableInlineResponse200194Api {
	return &NullableInlineResponse200194Api{value: val, isSet: true}
}

func (v NullableInlineResponse200194Api) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200194Api) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


