/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 04 March, 2026 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.68.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse20086Nat Configuration of NAT for the subnet
type InlineResponse20086Nat struct {
	// Whether or not VPN subnet translation is enabled for the subnet
	Enabled *bool `json:"enabled,omitempty"`
	// The translated subnet to be used in the VPN
	RemoteSubnet *string `json:"remoteSubnet,omitempty"`
}

// NewInlineResponse20086Nat instantiates a new InlineResponse20086Nat object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse20086Nat() *InlineResponse20086Nat {
	this := InlineResponse20086Nat{}
	return &this
}

// NewInlineResponse20086NatWithDefaults instantiates a new InlineResponse20086Nat object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse20086NatWithDefaults() *InlineResponse20086Nat {
	this := InlineResponse20086Nat{}
	return &this
}

// GetEnabled returns the Enabled field value if set, zero value otherwise.
func (o *InlineResponse20086Nat) GetEnabled() bool {
	if o == nil || isNil(o.Enabled) {
		var ret bool
		return ret
	}
	return *o.Enabled
}

// GetEnabledOk returns a tuple with the Enabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20086Nat) GetEnabledOk() (*bool, bool) {
	if o == nil || isNil(o.Enabled) {
    return nil, false
	}
	return o.Enabled, true
}

// HasEnabled returns a boolean if a field has been set.
func (o *InlineResponse20086Nat) HasEnabled() bool {
	if o != nil && !isNil(o.Enabled) {
		return true
	}

	return false
}

// SetEnabled gets a reference to the given bool and assigns it to the Enabled field.
func (o *InlineResponse20086Nat) SetEnabled(v bool) {
	o.Enabled = &v
}

// GetRemoteSubnet returns the RemoteSubnet field value if set, zero value otherwise.
func (o *InlineResponse20086Nat) GetRemoteSubnet() string {
	if o == nil || isNil(o.RemoteSubnet) {
		var ret string
		return ret
	}
	return *o.RemoteSubnet
}

// GetRemoteSubnetOk returns a tuple with the RemoteSubnet field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20086Nat) GetRemoteSubnetOk() (*string, bool) {
	if o == nil || isNil(o.RemoteSubnet) {
    return nil, false
	}
	return o.RemoteSubnet, true
}

// HasRemoteSubnet returns a boolean if a field has been set.
func (o *InlineResponse20086Nat) HasRemoteSubnet() bool {
	if o != nil && !isNil(o.RemoteSubnet) {
		return true
	}

	return false
}

// SetRemoteSubnet gets a reference to the given string and assigns it to the RemoteSubnet field.
func (o *InlineResponse20086Nat) SetRemoteSubnet(v string) {
	o.RemoteSubnet = &v
}

func (o InlineResponse20086Nat) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Enabled) {
		toSerialize["enabled"] = o.Enabled
	}
	if !isNil(o.RemoteSubnet) {
		toSerialize["remoteSubnet"] = o.RemoteSubnet
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse20086Nat struct {
	value *InlineResponse20086Nat
	isSet bool
}

func (v NullableInlineResponse20086Nat) Get() *InlineResponse20086Nat {
	return v.value
}

func (v *NullableInlineResponse20086Nat) Set(val *InlineResponse20086Nat) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse20086Nat) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse20086Nat) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse20086Nat(val *InlineResponse20086Nat) *NullableInlineResponse20086Nat {
	return &NullableInlineResponse20086Nat{value: val, isSet: true}
}

func (v NullableInlineResponse20086Nat) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse20086Nat) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


