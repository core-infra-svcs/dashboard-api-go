/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 04 February, 2026 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.67.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse200385MqttPublishing MQTT Publishing Settings
type InlineResponse200385MqttPublishing struct {
	// MQTT Publishing Frequency in seconds. Will be between 1 and 2147483647. Default is 1 second
	Frequency *int32 `json:"frequency,omitempty"`
	// MQTT Publishing QoS. Valid types are: 0, 1, 2
	Qos *int32 `json:"qos,omitempty"`
}

// NewInlineResponse200385MqttPublishing instantiates a new InlineResponse200385MqttPublishing object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200385MqttPublishing() *InlineResponse200385MqttPublishing {
	this := InlineResponse200385MqttPublishing{}
	return &this
}

// NewInlineResponse200385MqttPublishingWithDefaults instantiates a new InlineResponse200385MqttPublishing object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200385MqttPublishingWithDefaults() *InlineResponse200385MqttPublishing {
	this := InlineResponse200385MqttPublishing{}
	return &this
}

// GetFrequency returns the Frequency field value if set, zero value otherwise.
func (o *InlineResponse200385MqttPublishing) GetFrequency() int32 {
	if o == nil || isNil(o.Frequency) {
		var ret int32
		return ret
	}
	return *o.Frequency
}

// GetFrequencyOk returns a tuple with the Frequency field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200385MqttPublishing) GetFrequencyOk() (*int32, bool) {
	if o == nil || isNil(o.Frequency) {
    return nil, false
	}
	return o.Frequency, true
}

// HasFrequency returns a boolean if a field has been set.
func (o *InlineResponse200385MqttPublishing) HasFrequency() bool {
	if o != nil && !isNil(o.Frequency) {
		return true
	}

	return false
}

// SetFrequency gets a reference to the given int32 and assigns it to the Frequency field.
func (o *InlineResponse200385MqttPublishing) SetFrequency(v int32) {
	o.Frequency = &v
}

// GetQos returns the Qos field value if set, zero value otherwise.
func (o *InlineResponse200385MqttPublishing) GetQos() int32 {
	if o == nil || isNil(o.Qos) {
		var ret int32
		return ret
	}
	return *o.Qos
}

// GetQosOk returns a tuple with the Qos field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200385MqttPublishing) GetQosOk() (*int32, bool) {
	if o == nil || isNil(o.Qos) {
    return nil, false
	}
	return o.Qos, true
}

// HasQos returns a boolean if a field has been set.
func (o *InlineResponse200385MqttPublishing) HasQos() bool {
	if o != nil && !isNil(o.Qos) {
		return true
	}

	return false
}

// SetQos gets a reference to the given int32 and assigns it to the Qos field.
func (o *InlineResponse200385MqttPublishing) SetQos(v int32) {
	o.Qos = &v
}

func (o InlineResponse200385MqttPublishing) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Frequency) {
		toSerialize["frequency"] = o.Frequency
	}
	if !isNil(o.Qos) {
		toSerialize["qos"] = o.Qos
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200385MqttPublishing struct {
	value *InlineResponse200385MqttPublishing
	isSet bool
}

func (v NullableInlineResponse200385MqttPublishing) Get() *InlineResponse200385MqttPublishing {
	return v.value
}

func (v *NullableInlineResponse200385MqttPublishing) Set(val *InlineResponse200385MqttPublishing) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200385MqttPublishing) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200385MqttPublishing) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200385MqttPublishing(val *InlineResponse200385MqttPublishing) *NullableInlineResponse200385MqttPublishing {
	return &NullableInlineResponse200385MqttPublishing{value: val, isSet: true}
}

func (v NullableInlineResponse200385MqttPublishing) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200385MqttPublishing) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


