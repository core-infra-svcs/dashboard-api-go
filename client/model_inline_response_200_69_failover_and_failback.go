/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 02 April, 2025 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.57.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse20069FailoverAndFailback WAN failover and failback
type InlineResponse20069FailoverAndFailback struct {
	Immediate *InlineResponse20069FailoverAndFailbackImmediate `json:"immediate,omitempty"`
}

// NewInlineResponse20069FailoverAndFailback instantiates a new InlineResponse20069FailoverAndFailback object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse20069FailoverAndFailback() *InlineResponse20069FailoverAndFailback {
	this := InlineResponse20069FailoverAndFailback{}
	return &this
}

// NewInlineResponse20069FailoverAndFailbackWithDefaults instantiates a new InlineResponse20069FailoverAndFailback object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse20069FailoverAndFailbackWithDefaults() *InlineResponse20069FailoverAndFailback {
	this := InlineResponse20069FailoverAndFailback{}
	return &this
}

// GetImmediate returns the Immediate field value if set, zero value otherwise.
func (o *InlineResponse20069FailoverAndFailback) GetImmediate() InlineResponse20069FailoverAndFailbackImmediate {
	if o == nil || isNil(o.Immediate) {
		var ret InlineResponse20069FailoverAndFailbackImmediate
		return ret
	}
	return *o.Immediate
}

// GetImmediateOk returns a tuple with the Immediate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20069FailoverAndFailback) GetImmediateOk() (*InlineResponse20069FailoverAndFailbackImmediate, bool) {
	if o == nil || isNil(o.Immediate) {
    return nil, false
	}
	return o.Immediate, true
}

// HasImmediate returns a boolean if a field has been set.
func (o *InlineResponse20069FailoverAndFailback) HasImmediate() bool {
	if o != nil && !isNil(o.Immediate) {
		return true
	}

	return false
}

// SetImmediate gets a reference to the given InlineResponse20069FailoverAndFailbackImmediate and assigns it to the Immediate field.
func (o *InlineResponse20069FailoverAndFailback) SetImmediate(v InlineResponse20069FailoverAndFailbackImmediate) {
	o.Immediate = &v
}

func (o InlineResponse20069FailoverAndFailback) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Immediate) {
		toSerialize["immediate"] = o.Immediate
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse20069FailoverAndFailback struct {
	value *InlineResponse20069FailoverAndFailback
	isSet bool
}

func (v NullableInlineResponse20069FailoverAndFailback) Get() *InlineResponse20069FailoverAndFailback {
	return v.value
}

func (v *NullableInlineResponse20069FailoverAndFailback) Set(val *InlineResponse20069FailoverAndFailback) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse20069FailoverAndFailback) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse20069FailoverAndFailback) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse20069FailoverAndFailback(val *InlineResponse20069FailoverAndFailback) *NullableInlineResponse20069FailoverAndFailback {
	return &NullableInlineResponse20069FailoverAndFailback{value: val, isSet: true}
}

func (v NullableInlineResponse20069FailoverAndFailback) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse20069FailoverAndFailback) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


