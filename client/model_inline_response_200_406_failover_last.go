/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 04 February, 2026 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.67.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
	"time"
)

// InlineResponse200406FailoverLast Wireless LAN controller last failover information
type InlineResponse200406FailoverLast struct {
	// Wireless LAN controller last redundancy switchover time
	Ts *time.Time `json:"ts,omitempty"`
	// Wireless LAN controller last redundancy switchover reason
	Reason *string `json:"reason,omitempty"`
}

// NewInlineResponse200406FailoverLast instantiates a new InlineResponse200406FailoverLast object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse200406FailoverLast() *InlineResponse200406FailoverLast {
	this := InlineResponse200406FailoverLast{}
	return &this
}

// NewInlineResponse200406FailoverLastWithDefaults instantiates a new InlineResponse200406FailoverLast object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse200406FailoverLastWithDefaults() *InlineResponse200406FailoverLast {
	this := InlineResponse200406FailoverLast{}
	return &this
}

// GetTs returns the Ts field value if set, zero value otherwise.
func (o *InlineResponse200406FailoverLast) GetTs() time.Time {
	if o == nil || isNil(o.Ts) {
		var ret time.Time
		return ret
	}
	return *o.Ts
}

// GetTsOk returns a tuple with the Ts field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200406FailoverLast) GetTsOk() (*time.Time, bool) {
	if o == nil || isNil(o.Ts) {
    return nil, false
	}
	return o.Ts, true
}

// HasTs returns a boolean if a field has been set.
func (o *InlineResponse200406FailoverLast) HasTs() bool {
	if o != nil && !isNil(o.Ts) {
		return true
	}

	return false
}

// SetTs gets a reference to the given time.Time and assigns it to the Ts field.
func (o *InlineResponse200406FailoverLast) SetTs(v time.Time) {
	o.Ts = &v
}

// GetReason returns the Reason field value if set, zero value otherwise.
func (o *InlineResponse200406FailoverLast) GetReason() string {
	if o == nil || isNil(o.Reason) {
		var ret string
		return ret
	}
	return *o.Reason
}

// GetReasonOk returns a tuple with the Reason field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse200406FailoverLast) GetReasonOk() (*string, bool) {
	if o == nil || isNil(o.Reason) {
    return nil, false
	}
	return o.Reason, true
}

// HasReason returns a boolean if a field has been set.
func (o *InlineResponse200406FailoverLast) HasReason() bool {
	if o != nil && !isNil(o.Reason) {
		return true
	}

	return false
}

// SetReason gets a reference to the given string and assigns it to the Reason field.
func (o *InlineResponse200406FailoverLast) SetReason(v string) {
	o.Reason = &v
}

func (o InlineResponse200406FailoverLast) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Ts) {
		toSerialize["ts"] = o.Ts
	}
	if !isNil(o.Reason) {
		toSerialize["reason"] = o.Reason
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse200406FailoverLast struct {
	value *InlineResponse200406FailoverLast
	isSet bool
}

func (v NullableInlineResponse200406FailoverLast) Get() *InlineResponse200406FailoverLast {
	return v.value
}

func (v *NullableInlineResponse200406FailoverLast) Set(val *InlineResponse200406FailoverLast) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse200406FailoverLast) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse200406FailoverLast) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse200406FailoverLast(val *InlineResponse200406FailoverLast) *NullableInlineResponse200406FailoverLast {
	return &NullableInlineResponse200406FailoverLast{value: val, isSet: true}
}

func (v NullableInlineResponse200406FailoverLast) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse200406FailoverLast) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


