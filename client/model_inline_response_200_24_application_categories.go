/*
Meraki Dashboard API

The Cisco Meraki Dashboard API is a modern REST API based on the OpenAPI specification.  > Date: 07 February, 2024 > > [Recent Updates](https://meraki.io/whats-new/)  ---  [API Documentation](https://meraki.io/api)  [Community Support](https://meraki.io/community)  [Meraki Homepage](https://www.meraki.com) 

API version: 1.43.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// InlineResponse20024ApplicationCategories struct for InlineResponse20024ApplicationCategories
type InlineResponse20024ApplicationCategories struct {
	// The id of the category
	Id *string `json:"id,omitempty"`
	// The name of the category
	Name *string `json:"name,omitempty"`
	// Details of the associated applications
	Applications []InlineResponse20024Applications `json:"applications,omitempty"`
}

// NewInlineResponse20024ApplicationCategories instantiates a new InlineResponse20024ApplicationCategories object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInlineResponse20024ApplicationCategories() *InlineResponse20024ApplicationCategories {
	this := InlineResponse20024ApplicationCategories{}
	return &this
}

// NewInlineResponse20024ApplicationCategoriesWithDefaults instantiates a new InlineResponse20024ApplicationCategories object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInlineResponse20024ApplicationCategoriesWithDefaults() *InlineResponse20024ApplicationCategories {
	this := InlineResponse20024ApplicationCategories{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *InlineResponse20024ApplicationCategories) GetId() string {
	if o == nil || isNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20024ApplicationCategories) GetIdOk() (*string, bool) {
	if o == nil || isNil(o.Id) {
    return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *InlineResponse20024ApplicationCategories) HasId() bool {
	if o != nil && !isNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *InlineResponse20024ApplicationCategories) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *InlineResponse20024ApplicationCategories) GetName() string {
	if o == nil || isNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20024ApplicationCategories) GetNameOk() (*string, bool) {
	if o == nil || isNil(o.Name) {
    return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *InlineResponse20024ApplicationCategories) HasName() bool {
	if o != nil && !isNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *InlineResponse20024ApplicationCategories) SetName(v string) {
	o.Name = &v
}

// GetApplications returns the Applications field value if set, zero value otherwise.
func (o *InlineResponse20024ApplicationCategories) GetApplications() []InlineResponse20024Applications {
	if o == nil || isNil(o.Applications) {
		var ret []InlineResponse20024Applications
		return ret
	}
	return o.Applications
}

// GetApplicationsOk returns a tuple with the Applications field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InlineResponse20024ApplicationCategories) GetApplicationsOk() ([]InlineResponse20024Applications, bool) {
	if o == nil || isNil(o.Applications) {
    return nil, false
	}
	return o.Applications, true
}

// HasApplications returns a boolean if a field has been set.
func (o *InlineResponse20024ApplicationCategories) HasApplications() bool {
	if o != nil && !isNil(o.Applications) {
		return true
	}

	return false
}

// SetApplications gets a reference to the given []InlineResponse20024Applications and assigns it to the Applications field.
func (o *InlineResponse20024ApplicationCategories) SetApplications(v []InlineResponse20024Applications) {
	o.Applications = v
}

func (o InlineResponse20024ApplicationCategories) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !isNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !isNil(o.Applications) {
		toSerialize["applications"] = o.Applications
	}
	return json.Marshal(toSerialize)
}

type NullableInlineResponse20024ApplicationCategories struct {
	value *InlineResponse20024ApplicationCategories
	isSet bool
}

func (v NullableInlineResponse20024ApplicationCategories) Get() *InlineResponse20024ApplicationCategories {
	return v.value
}

func (v *NullableInlineResponse20024ApplicationCategories) Set(val *InlineResponse20024ApplicationCategories) {
	v.value = val
	v.isSet = true
}

func (v NullableInlineResponse20024ApplicationCategories) IsSet() bool {
	return v.isSet
}

func (v *NullableInlineResponse20024ApplicationCategories) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInlineResponse20024ApplicationCategories(val *InlineResponse20024ApplicationCategories) *NullableInlineResponse20024ApplicationCategories {
	return &NullableInlineResponse20024ApplicationCategories{value: val, isSet: true}
}

func (v NullableInlineResponse20024ApplicationCategories) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInlineResponse20024ApplicationCategories) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


